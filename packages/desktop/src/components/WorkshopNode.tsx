...
/**
 * WorkshopNode —— w 轴节点的 render function。
 *
 * 天然属于 w 轴（由 VisionPoint.axis === "w" 分发），无需 type 字段。
 * 持有 w 轴自身的视觉设计：
 * - `status: null`（uncertain，`•••`）是它的一个状态：无边框、无背景，
 *   仅渲染 `•••`；hover 时白底、无边框（与 origin 同色）。
 * - `status: VisionStatus`：按 STATUS_FILL 配色。
 * - `status: undefined`：透明底、常规边框。
 */
export function WorkshopNode({ q, r, s, id, status, label, children, onSelect }: WorkshopNodeProps) {
